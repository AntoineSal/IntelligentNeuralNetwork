...
# === CONFIGURATION (PTB FINAL) ===
CONFIG = {
    'dataset': 'ptb',
    'vocab_size': 10000,   # PTB standard
    'd_model': 256,
    'n_neurons': 32,       # On garde 32 pour la capacité
    'n_layers': 6,         # Profondeur pour raisonnement
    'dropout': 0.3,        # Dropout crucial sur PTB (petit dataset)
    'lr': 5e-4,            # Un peu plus agressif car petit dataset
    'batch_size': 32,
    'seq_len': 64,         # Standard pour PTB word-level
    'epochs': 20,          # Il faut iterer beaucoup sur PTB
    'grad_clip': 0.5,
    'save_every': 1000
}

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"=== LAUNCHING INNv2 FINAL ON PTB ===")
...
class PTBCorpus:
    ...
    def tokenize(self, path):
        if not os.path.exists(path):
            ...
            # Create dummy file if download failed to avoid crash
